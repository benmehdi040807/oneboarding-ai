...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // PayPal renvoie généralement subscription_id (Subscriptions v1). Parfois token/ba_token selon le flow.
    const subId =
    ...
    // Récupérer la souscription PayPal pour:
    // - récupérer custom_id = phoneE164
    // - déduire le plan via plan_id -> PLAN_IDS
    const pp = await ppGetSubscription(subId);

    // phoneE164 est placé dans custom_id lors du /api/pay/start
    ...
    // Déduire le plan interne depuis plan_id PayPal
    const planId: string | undefined = pp?.plan_id;
    let plan: PlanType = "CONTINU";
    if (planId && String(planId) === String(PLAN_IDS.PASS1MOIS)) plan = "PASS1MOIS";

    // Upsert local: rattacher cette souscription au user (création si besoin)
    ...
    // UX: renvoyer vers la home avec un flag
